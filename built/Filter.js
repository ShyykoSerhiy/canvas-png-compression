 ...
 */
function paethPredictor(left, above, uppperLeft) {
...
function filterNone(fromData, fromPos, byteWidth, filteredData, filteredPos) {
    PngWriter.copy(fromData, filteredData, filteredPos, byteWidth, fromPos); // just copy the data without filtering
}
function filterSumNone(fromData, fromPos, byteWidth) {
    var sum = 0;
...
function filterSub(fromData, fromPos, byteWidth, filteredData, filteredPos, bpp) {
    for (var i = 0; i < byteWidth; i++) {
        var left = i >= bpp ? fromData[fromPos + i - bpp] : 0;
        filteredData[filteredPos + i] = fromData[fromPos + i] - left;
    }
}
function filterSumSub(fromData, fromPos, byteWidth, bpp) {
    var sum = 0;
    for (var i = 0; i < byteWidth; i++) {
        var left = i >= bpp ? fromData[fromPos + i - bpp] : 0;
...
function filterUp(fromData, fromPos, byteWidth, filteredData, filteredPos) {
    for (var i = 0; i < byteWidth; i++) {
        var up = fromPos > 0 ? fromData[fromPos + i - byteWidth] : 0;
        filteredData[filteredPos + i] = fromData[fromPos + i] - up;
    }
}
function filterSumUp(fromData, fromPos, byteWidth) {
    var sum = 0;
    var length = fromPos + byteWidth;
    for (var i = fromPos; i < length; i++) {
...
function filterAvg(fromData, fromPos, byteWidth, filteredData, filteredPos, bpp) {
    for (var i = 0; i < byteWidth; i++) {
        var left = i >= bpp ? fromData[fromPos + i - bpp] : 0;
        var up = fromPos > 0 ? fromData[fromPos + i - byteWidth] : 0;
...
function filterSumAvg(fromData, fromPos, byteWidth, bpp) {
    var sum = 0;
    for (var i = 0; i < byteWidth; i++) {
        var left = i >= bpp ? fromData[fromPos + i - bpp] : 0;
        var up = fromPos > 0 ? fromData[fromPos + i - byteWidth] : 0;
...
function filterPaeth(fromData, fromPos, byteWidth, filteredData, filteredPos, bpp) {
    for (var i = 0; i < byteWidth; i++) {
        var left = i >= bpp ? fromData[fromPos + i - bpp] : 0;
        var up = fromPos > 0 ? fromData[fromPos + i - byteWidth] : 0;
...
function filterSumPaeth(fromData, fromPos, byteWidth, bpp) {
    var sum = 0;
    for (var i = 0; i < byteWidth; i++) {
        var left = i >= bpp ? fromData[fromPos + i - bpp] : 0;
        var up = fromPos > 0 ? fromData[fromPos + i - byteWidth] : 0;
...
export function filter(imageData) {
